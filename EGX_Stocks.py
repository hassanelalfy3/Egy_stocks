...
def add_indicators(df):
    ...
    # --- Bollinger Bands Calculation (Manual) ---
...
with st.spinner('جاري جلب البيانات...'):
    # Download 1 year of data to ensure indicators have enough history
...
if not df.empty and len(df) > 20:
    ...
    c2.metric("الهدف 1", rec["tp1"])
    c3.metric("الهدف 2", rec["tp2"])
    ...
    # Bands
    fig.add_trace(go.Scatter(x=df.index, y=df['Upper_Band'], name="المقاومة", line=dict(color='rgba(255,255,255,0.3)', dash='dot')), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['Lower_Band'], name="الدعم", line=dict(color='rgba(255,255,255,0.3)', dash='dot')), row=1, col=1)
    ...
    st.error("لا توجد بيانات كافية لهذا السهم.")
