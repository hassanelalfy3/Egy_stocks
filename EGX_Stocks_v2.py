...
def get_data_with_fallback(ticker, tf):
    # Standard map for the requested timeframe
    period_map = {
    ...
    try:
        ...
        # Attempt 2: Fallback to Hourly if empty
        if df.empty or len(df) < 5:
            df = yf.download(ticker, period="60d", interval="1h", progress=False, multi_level_index=False)
        
        # Attempt 3: Fallback to Daily (Last Resort for "Old" Data)
        if df.empty or len(df) < 5:
...
def plot_chart(df, ticker, strategy_name, current_params):
    ...
    if strategy_name == "VWAP + RSI Breakout":
        if 'VWAP' in df.columns: fig.add_trace(go.Scatter(x=df.index, y=df['VWAP'], name="VWAP", line=dict(color='orange')), row=1, col=1)
        if 'RSI' in df.columns: fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], name="RSI", line=dict(color='purple')), row=2, col=1)
    ...
    fig.update_layout(height=500, template="plotly_dark", xaxis_rangeslider_visible=False, title=f"Latest Data: {ticker}")
    return fig

# --- App Setup ---
st.set_page_config(page_title="Sniper Engine", layout="wide")
...
p_tf = st.sidebar.selectbox("Timeframe", ["1m", "5m", "15m", "30m", "1h", "1d", "1M", "YTD"], index=4)
...
if st.session_state.active:
    ...
    while True:
        ...
        for ticker in SCAN_LIST:
            ...
            if df.empty:
                results.append({"Ticker": ticker, "Status": "⚠️ Invalid Ticker", "Price": 0, "Details": "N/A", "Last Update": "N/A", "_ts": pd.Timestamp(0).tz_localize(CAIRO_TZ)})
                continue

            # Check if timeframe was switched internally
            is_fallback = False # You could add logic to detect if it's not the requested TF

            match, details = False, "N/A"
            if selected_strat == "VWAP + RSI Breakout":
                match, details = strategy_vwap_rsi(df, params.get('r_len', 14), params.get('r_thresh', 50))
            elif selected_strat == "MA Golden Cross":
                match, details = strategy_ma_cross(df, params.get('f_ma', 9), params.get('s_ma', 21))
